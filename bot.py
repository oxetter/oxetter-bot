...
if not BOT_TOKEN:
    print("[!] НЕТ TELEGRAM_TOKEN в переменных окружения")
    print("[!] Добавь его в Render → Environment")
...
async def cmd_start(message: types.Message):
    text = (
        ...
        "\n"
        "ПОДДЕРЖКА: PHONE, EMAIL, VK, TG, OK, WA, ADDRESS, PASSPORT, INN, SNILS, CAR, RELATIVE"
    )
...
async def handle_text(message: types.Message):
    ...
    try:
        ...
        if total == 0:
            await message.answer("[!] Не нашёл данных. Смотри /help")
            return
