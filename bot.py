...
CITIES_WORLD = [
    ...
    # ЕВРОПА
    "Лондон", "Манчестер", "Бирмингем", "Ливерпуль", "Лидс", "Бристоль",
    ...
    "Афины", "Салоники", "Патрас", "Ираклион",
...
def format_admin_stats(s):
    ...
    t += "  ⭐ всего: " + str(s["stars_total"]) + "\n"
    t += "  ⭐ сегодня: +" + str(s["stars_today"]) + "\n\n"
    t += "🔑 КЛЮЧИ:\n"
...
def country_by_phone(phone):
    ...
    if digits.startswith("380"): return "Украина"
    if digits.startswith("375"): return "Беларусь"
    if digits.startswith("373"): return "Молдова"
    if digits.startswith("994"): return "Азербайджан"
    if digits.startswith("374"): return "Армения"
    if digits.startswith("995"): return "Грузия"
    if digits.startswith("998"): return "Узбекистан"
    if digits.startswith("996"): return "Кыргызстан"
    if digits.startswith("992"): return "Таджикистан"
    if digits.startswith("993"): return "Туркменистан"
    if digits.startswith("1"): return "США/Канада"
    if digits.startswith("44"): return "Великобритания"
    if digits.startswith("49"): return "Германия"
    if digits.startswith("33"): return "Франция"
    if digits.startswith("39"): return "Италия"
    if digits.startswith("34"): return "Испания"
    if digits.startswith("48"): return "Польша"
    if digits.startswith("31"): return "Нидерланды"
    if digits.startswith("32"): return "Бельгия"
    if digits.startswith("41"): return "Швейцария"
    if digits.startswith("43"): return "Австрия"
    if digits.startswith("46"): return "Швеция"
    if digits.startswith("47"): return "Норвегия"
    if digits.startswith("45"): return "Дания"
    if digits.startswith("358"): return "Финляндия"
    if digits.startswith("351"): return "Португалия"
    if digits.startswith("30"): return "Греция"
    if digits.startswith("420"): return "Чехия"
    if digits.startswith("421"): return "Словакия"
    if digits.startswith("36"): return "Венгрия"
    if digits.startswith("40"): return "Румыния"
    if digits.startswith("359"): return "Болгария"
    if digits.startswith("381"): return "Сербия"
    if digits.startswith("385"): return "Хорватия"
    if digits.startswith("387"): return "Босния"
    if digits.startswith("382"): return "Черногория"
    if digits.startswith("386"): return "Словения"
    if digits.startswith("370"): return "Литва"
    if digits.startswith("371"): return "Латвия"
    if digits.startswith("372"): return "Эстония"
    if digits.startswith("86"): return "Китай"
    if digits.startswith("81"): return "Япония"
    if digits.startswith("82"): return "Корея"
    if digits.startswith("91"): return "Индия"
    if digits.startswith("92"): return "Пакистан"
    if digits.startswith("880"): return "Бангладеш"
    if digits.startswith("84"): return "Вьетнам"
    if digits.startswith("66"): return "Таиланд"
    if digits.startswith("60"): return "Малайзия"
    if digits.startswith("65"): return "Сингапур"
    if digits.startswith("62"): return "Индонезия"
    if digits.startswith("63"): return "Филиппины"
    if digits.startswith("90"): return "Турция"
    if digits.startswith("972"): return "Израиль"
    if digits.startswith("971"): return "ОАЭ"
    if digits.startswith("966"): return "Саудовская Аравия"
    if digits.startswith("974"): return "Катар"
    if digits.startswith("965"): return "Кувейт"
    if digits.startswith("973"): return "Бахрейн"
    if digits.startswith("968"): return "Оман"
    if digits.startswith("20"): return "Египет"
    if digits.startswith("27"): return "ЮАР"
    if digits.startswith("234"): return "Нигерия"
    if digits.startswith("254"): return "Кения"
    if digits.startswith("55"): return "Бразилия"
    if digits.startswith("54"): return "Аргентина"
    if digits.startswith("56"): return "Чили"
    if digits.startswith("57"): return "Колумбия"
    if digits.startswith("52"): return "Мексика"
    if digits.startswith("61"): return "Австралия"
    if digits.startswith("64"): return "Новая Зеландия"
    return None
...
def country_by_city(city):
    ...
    if cl in CITIES_LOWER:
        ru_cities = ["москва", "санкт-петербург", "спб", "новосибирск", "екатеринбург",
                     "казань", "нижний новгород", "челябинск", "самара", "омск"]
        if cl in ru_cities:
            return "Россия"
        ua_cities = ["киев", "харьков", "одесса", "днепр", "донецк", "запорожье",
                     "львов", "николаев", "мариуполь", "луганск"]
        if cl in ua_cities:
            return "Украина"
        by_cities = ["минск", "гомель", "могилёв", "витебск", "гродно", "брест"]
        if cl in by_cities:
            return "Беларусь"
        kz_cities = ["алматы", "астана", "шымкент", "караганда", "актобе", "тараз",
                     "павлодар", "семей", "атырау", "костанай", "актау"]
        if cl in kz_cities:
            return "Казахстан"
...
# ============ OSINT: ФИО (СВОБОДНЫЙ ВВОД + ВЕСЬ МИР) ============
def fio_advanced(raw_input):
    ...
    fio = parsed.get("fio")
    parts = []
    if parsed.get("lastname"): parts.append(parsed["lastname"])
    if parsed.get("firstname"): parts.append(parsed["firstname"])
    if parsed.get("middlename"): parts.append(parsed["middlename"])
...
def render_graph(data):
    ...
    if t.get("dob"): target_lines.append("[DR] " + str(t["dob"]))
    if t.get("phone"): target_lines.append("[TEL] " + str(t["phone"]))
    if t.get("telegram"): target_lines.append("[TG] " + str(t["telegram"]))
    if t.get("tgid"): target_lines.append("[ID] " + str(t["tgid"]))
    if t.get("email"): target_lines.append("[MAIL] " + str(t["email"]))
    if t.get("address"): target_lines.append("[ADR] " + str(t["address"])[:30])
    if t.get("passport"): target_lines.append("[PASS] " + str(t["passport"]))
    if t.get("inn"): target_lines.append("[INN] " + str(t["inn"]))
    if t.get("snils"): target_lines.append("[SNILS] " + str(t["snils"]))
    ...
    if relatives and target_bbox:
        ...
        for idx, rel in enumerate(relatives):
            ...
            if rdata.get("dob"): rel_lines.append("[DR] " + str(rdata["dob"]))
            if rdata.get("phone"): rel_lines.append("[TEL] " + str(rdata["phone"]))
            if rdata.get("telegram"): rel_lines.append("[TG] " + str(rdata["telegram"]))
            if rdata.get("email"): rel_lines.append("[MAIL] " + str(rdata["email"]))
            if rdata.get("address"): rel_lines.append("[ADR] " + str(rdata["address"])[:25])
...
1) Иванов Иван Владимирович
   (можно без фамилии или отчества)
...
12) Иванов Иван, Москва, +7 999 123-45-67
    (с запятыми и пробелами в номере)

13) Иванов Иван (1999) Лондон
    (год + зарубежный город)
...
• Города: РФ, СНГ, Европа, Азия, Америка, Африка, Океания
• Телефоны: +7, +380, +375, +1, +44, +49, +33, +39, +34, +86, +81, +82, +91, +90, +972, +971 и др.
• Email: любые домены (.ru, .com, .de, .uk, .cn и т.д.)
• Форматы даты: ДД.ММ.ГГГГ, ММ/ДД/ГГГГ, ГГГГ-ММ-ДД, (ГГГГ)

Отправь строку — бот сам всё разберёт."""

EXAMPLE_GRAPH = """📝 ПРИМЕР ДЛЯ ГРАФА СВЯЗЕЙ

Можно отправить многострочно с ролями:
...
if bot:
    ...
    def cb_back(call):
        ...
        bot.edit_message_text("Меню:", call.message.chat.id, call.message.message_id, reply_markup=main_menu_inline(call.from_user.id))
    ...
    def cb_search(call):
        bot.answer_callback_query(call.id)
        bot.edit_message_text("🔍 Тип поиска:", call.message.chat.id, call.message.message_id, reply_markup=search_menu_inline())
    ...
    def process_search_fio(message):
        ...
        try:
            ...
            text += "📊 РАЗБОР:\n"
            if parsed.get("lastname"): text += "  Фамилия: " + parsed["lastname"] + "\n"
            if parsed.get("firstname"): text += "  Имя: " + parsed["firstname"] + "\n"
            if parsed.get("middlename"): text += "  Отчество: " + parsed["middlename"] + "\n"
            if parsed.get("dob"): text += "  🎂 ДР: " + parsed["dob"] + "\n"
            if parsed.get("birth_year"): text += "  📅 Год: " + parsed["birth_year"] + "\n"
            if parsed.get("age"): text += "  👤 Возраст: " + str(parsed["age"]) + "\n"
            if parsed.get("city"): text += "  🏙 Город: " + parsed["city"] + "\n"
            if parsed.get("country"): text += "  🌍 Страна: " + parsed["country"] + "\n"
            if parsed.get("phone"): text += "  📱 Номер: " + parsed["phone"] + "\n"
            if parsed.get("email"): text += "  📧 Email: " + parsed["email"] + "\n"
            if parsed.get("nick"): text += "  👤 Ник: @" + parsed["nick"] + "\n"
            if p.get("gender"): text += "  ⚧ Пол: " + p["gender"] + "\n"
            if p.get("nationality"): text += "  🌐 Нация: " + p["nationality"] + "\n"
            text += "\n"
 ...
 send_menu(message.chat.id, uid)
    ...
    def process_search_ip(message):
        ...
        try:
            ...
            if not r.get("valid"):
                ...
                return
            text = "📡 IP: " + r["input"] + "\n\n"
            if r.get("country"): text += "🌍 " + str(r["country"]) + "\n"
            if r.get("region"): text += "🏙 " + str(r["region"]) + "\n"
            if r.get("city"): text += "🏘 " + str(r["city"]) + "\n"
            if r.get("lat"): text += "📍 " + str(r["lat"]) + ", " + str(r["lon"]) + "\n"
            if r.get("isp"): text += "📶 " + str(r["isp"]) + "\n"
            if r.get("as"): text += "🔢 " + str(r["as"]) + "\n"
            bot.send_message(message.chat.id, text)
        except Exception as e:
            bot.send_message(message.chat.id, "❌ Ошибка: " + str(e)[:200])
        send_menu(message.chat.id, uid)
    ...
    def process_search_exif(message):
        ...
        try:
            ...
            text = "📷 EXIF ФОТО\n\n"
            info = r.get("info", {})
            if info.get("Make"): text += "📱 " + info["Make"] + "\n"
            if info.get("Model"): text += "📱 " + info["Model"] + "\n"
            if info.get("Software"): text += "💻 " + info["Software"] + "\n"
