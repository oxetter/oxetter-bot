...
def parse_input(text):
    """Умный парсер. TARGET не обязателен."""
...
def generate_obsidian_canvas(data):
    ...
    def add_node(node_id, text, x, y, color="1"):
        canvas["nodes"].append({"id": node_id, "type": "text", "text": text, "x": x, "y": y, "width": 400, "height": 300, "color": color})

    def add_edge(from_id, to_id, label=""):
        canvas["edges"].append({"id": "edge_" + str(len(canvas["edges"])), "fromNode": from_id, "fromSide": "right", "toNode": to_id, "toSide": "left", "label": label})
...
def phone_advanced(phone):
    ...
    try:
        ...
        if "tgme_page_title" in rq.text:
            ...
            r["sections"]["socials"]["telegram"] = {"found": True, "name": m.group(1).strip() if m else "есть", "link": link}
        else:
    ...
    try:
        ...
        r["sections"]["socials"]["whatsapp"] = {"found": "WhatsApp" in rq.text and "not on WhatsApp" not in rq.text, "link": wa_link}
    except Exception:
        pass

    r["sections"]["dorks"] = [
        f'site:pastebin.com "{phone_clean}"',
        f'site:telegram.org "{phone_clean}"',
        f'"{phone_clean}" объявление',
        f'"{phone_clean}" утечка',
        f'site:vk.com "{phone_clean}"',
    ]
...
# ============ OSINT: ФИО ============
def fio_advanced(fio, dob=None, city=None):
    r = {"input": fio, "dob": dob, "city": city, "sections": {}}
    ...
    if len(parts) >= 3:
        ...
        if "енко" in l or l.endswith("ук"):
            r["sections"]["parsed"]["nationality"] = "Украинская"
        elif "ян" in l:
            r["sections"]["parsed"]["nationality"] = "Армянская"
        elif "дзе" in l or "швили" in l:
            r["sections"]["parsed"]["nationality"] = "Грузинская"
        elif any(s in l for s in ["ов", "ев", "ин", "ский", "цкий"]):
            r["sections"]["parsed"]["nationality"] = "Русская/славянская"
    ...
    dorks = [
        ...
        f'site:ok.ru "{fio}"',
        f'site:t.me "{fio}"',
        f'site:instagram.com "{fio}"',
        f'"{fio}" резюме',
        f'"{fio}" работа',
    ]
    if city:
        dorks.append(f'"{fio}" "{city}"')
        dorks.append(f'site:vk.com "{fio}" "{city}"')
    if dob:
        dorks.append(f'"{fio}" "{dob}"')
...
if bot:
    ...
    def process_mirror(message):
        ...
        if create_mirror_request(uid, token):
            ...
            bot.send_message(message.chat.id, "✅ Заявка принята! +12 часов.")
            if ADMIN_ID:
                try:
    ...
    def cb_search_fio(call):
        bot.answer_callback_query(call.id)
        msg = bot.send_message(call.message.chat.id, "📛 Введи ФИО, ДР, город через запятую:\n\nПример: Иванов Иван, 12.05.1995, Москва")
    ...
    def process_search_fio(message):
        ...
        fio = parts[0]
        dob = parts[1] if len(parts) > 1 else None
        city = parts[2] if len(parts) > 2 else None
        bot.send_message(message.chat.id, "🔍 Расширенный поиск...")
        try:
            r = fio_advanced(fio, dob, city)
            text = "📛 ФИО: " + fio + "\n"
            text += "━━━━━━━━━━━━━━━━━━━━\n\n"
            parsed = r["sections"].get("parsed", {})
            if parsed:
                text += "📊 РАЗБОР:\n"
                if parsed.get("gender"): text += "  ⚧ Пол: " + parsed["gender"] + "\n"
                if parsed.get("nationality"): text += "  🌐 Нация: " + parsed["nationality"] + "\n"
                text += "\n"
