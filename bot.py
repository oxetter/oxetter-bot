...
FIELD_MARKERS = {
    ...
    "номер": "phone", "phone": "phone", "телефон": "phone",
    "tg": "telegram", "telegram": "telegram", "тг": "telegram",
    "tgid": "tgid", "айди": "tgid", "id": "tgid",
    "vk": "vk", "вк": "vk",
    "ок": "ok", "ok": "ok",
    "wa": "whatsapp", "whatsapp": "whatsapp", "ватсап": "whatsapp",
    "max": "max", "макс": "max",
    "адрес": "address", "address": "address",
    "дата рождения": "dob", "др": "dob", "dob": "dob", "дата": "dob",
    "email": "email", "почта": "email",
    "имя": "name", "фио": "name", "name": "name",
}
...
def detect_field(value):
    ...
    # Только цифры, пробелы, дефисы — документ или телефон
    if re.match(r"^[\d\s\-\+\(\)]+$", v):
        digits_only = re.sub(r"[^\d]", "", v)

        # Телефон — есть + или начинается с 7/8 и 11 цифр
    ...
    # TG ID (8-12 цифр) — после документов
...
def parse_input(text):
    ...
    for line in lines:
        # Проверка на маркер роли или поля
        ...
        # Обычная строка — определяем тип
        if current_role is not None:
            ftype = detect_field(line)
            if ftype:
                current_data[ftype] = line
            else:
